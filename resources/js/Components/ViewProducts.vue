...
<template>
    <table class="font-roboto table-auto text-left text-[#6E6E6F]">
      <tr class="border-b-2 border-gray-300">
        <td class="py-3 pl-4 lg:pl-8 lg:w-72">АРТИКУЛ</td>
        <td class="w-64">НАЗВАНИЕ</td>
        <td class="w-64">СТАТУС</td>
        <td class="w-64">АТРИБУТЫ</td>
     </tr>
     <tr class="border-b-2 border-gray-300 bg-white text-sm lg:text-lg hover:cursor-pointer" v-for="product in $page.props.products" :key="product.id" @click="openProductModal(product)">
        <td class="py-2 px-2 lg:py-8 lg:pl-8 lg:w-72">{{ product.article }}</td>
        <td>{{ product.name }}</td>
    ...
    <Modal :show="showOpenProductModal" @close="closeProductModal">
      <div class="px-3 lg:py-5 h-96 bg-[#374050] text-white font-roboto">
            ...
            <div class="flex">
                <p class="text-[11px] text-[#ffffff99] lg:w-20">Артикул</p>
                <p class="text-[11px] text-[#ffffff]">{{ view.article }}</p>
            </div>
            <div class="flex mt-2">
                <p class="text-[11px] text-[#ffffff99] lg:w-20">Название</p>
                <p class="text-[11px] text-[#ffffff]">{{ view.name }}</p>
            </div>
            <div class="flex mt-2">
                <p class="text-[11px] text-[#ffffff99] lg:w-20">Статус</p>
                <p class="text-[11px] text-[#ffffff]">{{ view.status }}</p>
            </div>
            <div class="flex mt-2">
                <p class="text-[11px] text-[#ffffff99] lg:w-20">Атрибуты</p>
